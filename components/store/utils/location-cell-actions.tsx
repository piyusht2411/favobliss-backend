...
interface LocationCellActionsProps {
  data: LocationColumn;
}

export const LocationCellActions = ({ data }: LocationCellActionsProps) => {
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.storeId}/location/${data.id}`);
      router.refresh();
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-44">
          ...
          <DropdownMenuItem
            onClick={() =>
              copyToClipboard(data.id, "Location ID is copied to clipboard")
            }
          >
          ...
          <DropdownMenuItem
            onClick={() =>
              router.push(`/${params.storeId}/location/${data.id}`)
            }
          >
